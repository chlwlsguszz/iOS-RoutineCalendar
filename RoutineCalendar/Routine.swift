...
class Routine /*NSObject, NSCoding*/{
    ...
    init(date: Date, owner: String?, kind: Kind, content: String, emoji: String){
...
extension Routine{
    convenience init(date: Date? = nil, withData: Bool = false){
        if withData == true{
            ...
            self.init(date: date ?? Date(), owner: "me", kind: kind, content: content, emoji: "🏅")
            
        }else{
            self.init(date: date ?? Date(), owner: "me", kind: .Etc, content: "", emoji: "🏅")
