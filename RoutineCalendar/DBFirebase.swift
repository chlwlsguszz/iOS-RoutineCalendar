//
//  DBFirebase.swift
//  ch12-ChoiJinHyun-sharedPlan
//
//  Created by 최진현 on 2023/05/22.
//
...
class DbFirebase: Database {
    ...
    var parentNotification: ((Routine?, DbAction?) -> Void)? // PlanGroupViewController에서 설정
    var existQuery: ListenerRegistration?                 // 이미 설정한 Query의 존재여부
    
    required init(parentNotification: ((Routine?, DbAction?) -> Void)?) {
        self.parentNotification = parentNotification
        reference = Firestore.firestore().collection("routines") // 첫번째 "plans"라는 Collection
    }
}
...
extension DbFirebase{
    ...
    func saveChange(routine: Routine, action: DbAction){
        if action == .Delete{
            reference.document(routine.key).delete()    // key로된 plan을 지운다
            return
        }
        // plan을 아카이빙한다.
        ...
        reference.document(routine.key).setData(storeDate)
    }
}

extension DbFirebase{
    
    func queryRoutine(fromDate: Date, toDate: Date) {
